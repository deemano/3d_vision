...
:: Unzip 'libs' folder into its place
IF EXIST "%~dp0libs.zip" (
    ECHO.  
    ECHO  Unzipping 'libs' folder...
    PowerShell -Command "Expand-Archive -LiteralPath '%~dp0libs.zip' -DestinationPath '%~dp0' -Force"
)
...
echo  ^Copyright: Liverpool Hope University, 2024 - Denis Manolescu
...
IF EXIST "%scriptPath%libs\%opencvFileName%" (
    ...
    ECHO  Please ignore any extraction errors - those are are generated by source code changes of OpenCV.
    :: Execute the installer
    START /wait "" "%scriptPath%libs\%opencvFileName%" /VERYSILENT /DIR="%scriptPath%libs\opencv" /qb
